...
impl MockPool {
    ...
    pub fn is_whitelisted_for_reserve(env: Env, _asset: Address, user: Address) -> bool {
        // Check if there's a whitelist stored for this user
        // Default behavior: empty whitelist = open access (returns true)
        // This matches the real pool contract behavior where empty whitelist allows all
        ...
        match whitelist {
            ...
            _ => {
                // Empty whitelist = open access (matches real contract behavior)
                true
            }
        }
    }
    ...
    /// WP-L9: Mock get_current_liquidity_index — aToken reads index from pool
    ...
    /// Helper to set the mock liquidity index
...
fn test_mint_scaled() {
    ...
    let result = client.mint_scaled(&mock_pool, &user, &amount, &index);
...
fn test_burn_scaled() {
    ...
    let result = client.burn_scaled(&mock_pool, &user, &burn_amount, &index);
    assert_eq!(result, true);
...
fn test_interest_accrual_multiple_operations() {
    ...
    // Index increases to 1.1 (10% interest) — via mock pool
...
fn test_transfer_preserves_total_supply_with_interest() {
    ...
    mock_pool_client.set_liquidity_index(&(RAY + RAY / 10));
...
fn test_balance_of_with_liquidity_index() {
    ...
    // Deploy aToken
    let atoken_id = env.register(ATokenContract, ());
    let client = ATokenContractClient::new(&env, &atoken_id);
...
fn test_pool_calls_atoken_balance() {
    ...
    let underlying = Address::generate(&env);
    let pool = Address::generate(&env);
    ATokenContractClient::new(&env, &atoken_id).initialize(
        &pool,
        &underlying,
        &pool,
        &String::from_str(&env, "aXLM"),
    ...
    let user = Address::generate(&env);
    let index = RAY;
    ATokenContractClient::new(&env, &atoken_id).mint_scaled(&pool, &user, &1000_u128, &index);
